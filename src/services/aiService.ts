...
export async function getAIPredictions(request: AIPredictionRequest): Promise<AIAnalysisResponse> {
  try {
    logger.log('🤖 [AI Service] Making predictions request:', {
    ...
    logger.log('✅ [AI Service] Predictions response received:', {
    ...
    logger.error('❌ [AI Service] AI prediction error:', error);
...
export async function getAIAnalysis(seasonId: number): Promise<AIAnalysisResponse> {
  try {
    logger.log(`📋 [AI Service] Getting cached analysis for season ${seasonId}:`, `${API_BASE_URL}/ai/analysis/${seasonId}?type=predictions`);
    
    const response = await axios.get(`${API_BASE_URL}/ai/analysis/${seasonId}?type=predictions`);
    
    logger.log('✅ [AI Service] Cached analysis response received:', {
      status: response.status,
      hasData: !!response.data,
      dataKeys: Object.keys(response.data || {})
    });
    ...
    logger.log(`❌ [AI Service] Cache check failed for season ${seasonId}:`, {
    ...
    if (error.response?.status === 404) {
      logger.log(`📋 [AI Service] Cache miss for season ${seasonId} - this is expected`);
      throw new Error('CACHE_MISS');
    } else {
      logger.error('❌ [AI Service] AI analysis error:', error);
...
export async function getMetaHealthAnalysis(request: MetaHealthRequest): Promise<MetaHealthResponse> {
  logger.log('🤖 [AI Service] getMetaHealthAnalysis called', {
    seasonId: request.seasonId
  });
  ...
  try {
    ...
    if (request.forceRefresh) {
      logger.log(`🔄 [AI Service] Force refresh enabled for season ${request.seasonId} - generating new analysis`);
      const response = await axios.post(`${API_BASE_URL}/ai/meta-health`, request, {
        headers: { 'Content-Type': 'application/json' },
      });
      logger.log('✅ [AI Service] Meta health analysis (forced) completed:', {
        status: response.status,
        hasData: !!response.data,
        dataKeys: Object.keys(response.data || {})
      });
    ...
    logger.log(`📋 [AI Service] Checking cache for season ${request.seasonId}`);
    const cachedResponse = await axios.get(`${API_BASE_URL}/ai/analysis/${request.seasonId}?type=meta_health`);
    logger.log('✅ [AI Service] Cached meta health analysis found:', {
    ...
    logger.log(`📋 [AI Service] Cache miss for season ${request.seasonId} - generating new analysis`);
    
    // If cache miss (404), call the meta-health endpoint to generate new analysis
    if (error.response?.status === 404) {
      logger.log('🤖 [AI Service] Calling meta-health endpoint for new analysis...');
      ...
      logger.log('✅ [AI Service] Meta health analysis completed:', {
        status: response.status,
        hasData: !!response.data,
        dataKeys: Object.keys(response.data || {})
      });
      ...
      logger.error('❌ [AI Service] Meta health analysis error:', error);
      if (error.response) {
